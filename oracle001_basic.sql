...
별칭(alias)에 공백이 있을때는 " "(쌍따옴표)를 지정한다.
*/
...
-- DISTINCT 은 중복제거를 한 후 출력해 주는 명령어이다.
...
-- employees테이블에서 salary가 3000미만 일때의
...
-- employees테이블에서 first_name컬럼의 값이 'David'가 아닐때
-- first_name, salary를 출력하라.
SELECT first_name, salary
FROM employees
WHERE first_name != 'David';
-- WHERE first_name <> 'David'; 같은 의미


-- &&(and), ||(or)
-- employees테이블에서 salary가 3000, 9000, 17000 일때
-- first_name, hire_date, salary를 출력하라.
SELECT first_name, hire_date, salary
FROM employees
WHERE salary=3000 OR salary=9000 OR salary=17000;
-- WHERE salary IN(3000, 9000, 17000); 같은 의미

-- employees테이블에서 salary가 3000부터 5000까지 일때
-- first_name, hire_date, salary를 출력하라.
SELECT first_name, hire_date, salary
FROM employees
WHERE salary >= 3000 AND salary <= 5000;
-- WHERE salary BETWEEN 3000 AND 5000; 같은 의미 (3000이상 5000이하, 포함될때)
...
-- WHERE job_id <> 'IT_PROG';
-- WHERE job_id ^= 'IT_PROG';
-- WHERE NOT(job_id = 'IT_PROG');  같은의미


-- employees테이블에서 salary가 3000, 9000, 17000이 아닐때
-- first_name, hire_date, salary를 출력하라.
SELECT first_name, hire_date, salary
FROM employees
WHERE NOT(salary=3000 OR salary=9000 OR salary=17000);
-- WHERE salary NOT IN(3000, 9000, 17000); 같은 의미


-- employees테이블에서 salary가 3000부터 5000까지가 아닐때
-- first_name, hire_date, salary를 출력하라.
SELECT first_name, hire_date, salary
FROM employees
WHERE NOT(salary >= 3000 AND salary <= 5000);
-- WHERE salary NOT BETWEEN 3000 AND 5000; 같은 의미
...
-- employees테이블에서 commission_pct가 null이 아닐때
-- first_name, salary, commission_pct를 출력하라.
SELECT first_name, salary, commission_pct
FROM employees
...
-- WHERE first_name LIKE '%der%';  der이 포함된
-- WHERE first_name LIKE '%der'; der로 끝나는
-- WHERE first_name LIKE 'der%'; der로 시작하는


-- employees테이블에서 first_name의 값중 'A'로 시작하고
-- 두번째 문자는 임의문자이면 'exander'로 끝나는
-- first_name, salary, email을 출력하라.
SELECT first_name, salary, email
FROM employees
...
-- 사원의 first_name, salary, hire_date를 출력하라.
...
-- 급여(salary)가 높은순으로
-- first_name, job_id, salary를 출력하라.
